...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.body.addEventListener('click', async function(event) {
        ...
        if (target.matches('.pm-post-action-like, .pm-post-action-like *')) {
            ...
            const data = await apiCall(`/post/${postId}/like`, 'POST', null, csrfToken);
            if (data) {
            ...
            const postId = button.dataset.postId;
            if (!postId) return;
            const data = await apiCall(`/post/${postId}/dislike`, 'POST', null, csrfToken);
            if (data) {
                if (typeof showToast === 'function') showToast(data.message || `Post ${data.action}!`, data.status === 'success' ? 'success' : 'info', 1500);
                const postContainer = button.closest('.pm-post-item-container');
                if (postContainer) {
        ...
        // --- Share Post Button Handler ---
            ...
            const data = await apiCall(`/post/${postIdToShare}/share`, 'POST', null, csrfToken);
            if (data) {
    ...
    document.body.addEventListener('submit', async function(event) {
        ...
        if (form.matches('.pm-post-main-comment-form')) { // Updated selector
            ...
            const data = await apiCall(`/post/${postId}/comment`, 'POST', { text_content: textContent }, csrfToken);
